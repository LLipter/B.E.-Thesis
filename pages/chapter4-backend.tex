...
我们将通过JavaMail库的帮助与一个SMTP服务器建立联系，并最终将邮件推送到用户的设备上。
...
发送电子邮件因为涉及网络IO交互，所以相比于其他操作会产生巨大的延迟。为了提高响应速度，让用户拥有更好的使用体验，我们利用Spring Boot的@Async关键词对本模块做了一层封装修饰。让发送方式从同步阻塞变成异步非阻塞，增加效率，显著减少了用户需要等待的时间。但这样做有利有弊，缺点是即使服务器端发生错误（如500内部错误），导致电子邮件发送失败，用户也无法得到相应的错误提示，用户会错以为我们已经正确的发送了电子邮件。
